...
REM
REM The webui-only builds (webui_only, webui_wv) exclude illwill
REM (TUI library), resulting in smaller binaries with no
REM terminal UI dependency.
REM ============================================================
...
:wv
echo.
echo === Building webui_wv.exe (WebUI + WebView2) ===
echo.
echo [INFO] Requires winim and WebView2 runtime
echo.
nim c %WVFLAGS% --out:"webui_wv.exe" webui_wv.nim
if %ERRORLEVEL% neq 0 (
  echo.
  echo [ERROR] webui_wv build failed! (code %ERRORLEVEL%)
  exit /b %ERRORLEVEL%
)
echo.
echo [OK] webui_wv.exe created
goto end
